	...
	override doGenerate(Resource resource, IFileSystemAccess fsa) {
		...
		public class «fileName» extends org.eclipse.xtext.scoping.impl.AbstractDeclarativeScopeProvider {
			...
			static {
				...
				referencesMap = map.computeIfAbsent("«entry.key.EClass.name»", key -> new HashMap<>());
				eClassesMap = referencesMap.computeIfAbsent("«entry.key.EReference.name»", key -> new HashMap<>());
