...
class OCL2ValidatorGenerator implements IGenerator {
	...
	override doGenerate(Resource resource, IFileSystemAccess fsa) {
		...
		for (classOclExpressionEntry : classOclExpressions.entrySet().filter(e | e.value.size() > 0)) {
			...
			val packageClassName = classOclExpressionEntry.key.EPackage.name.toFirstUpper + "Package";
			...
			for (oclExpressionEntry : classOclExpressionEntry.value.entrySet()) {
				...
				fsa.generateFile(
					...
					public class «evaluatorClassName» implements at.ac.tuwien.big.oclgen.OCLBooleanExpressionEvaluator<«className»> {
						
						public static final at.ac.tuwien.big.oclgen.OCLBooleanExpressionEvaluator<«className»> INSTANCE = new «evaluatorClassName»();
			...
			fsa.generateFile(
				...
				public class «validatorClassName» extends org.eclipse.xtext.validation.AbstractDeclarativeValidator {
					...
					override java.util.List<EPackage> getEPackages() {
					    ...
					    result.add(EPackage.Registry.INSTANCE.getEPackage("«classOclExpressionEntry.key.EPackage.nsURI»"));
						return result;
					}
					
					def static EPackage getPackage() {
						return EPackage.Registry.INSTANCE.getEPackage("«classOclExpressionEntry.key.EPackage.nsURI»");
					}
					«FOR oclExpressionEntry : classOclExpressionEntry.value.entrySet()»
					
					public static final String «oclExpressionEntry.key.toUpperCase»_ISSUE_KEY = "«oclExpressionEntry.key»Error";
					private static final at.ac.tuwien.big.oclgen.OCLBooleanExpressionEvaluator<«className»> evaluator«oclExpressionEntry.key» = «packageName».«oclExpressionEntry.key»BooleanExpressionEvaluator.INSTANCE;
