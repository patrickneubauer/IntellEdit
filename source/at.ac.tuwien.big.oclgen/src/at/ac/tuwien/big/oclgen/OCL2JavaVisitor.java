	...
	public String visitOperationCallExp(OperationCallExp<EClassifier, EOperation> callExp) {
		...
		case PredefinedType.EQUAL:
			if (callExp.getSource().getType().getInstanceClass().isPrimitive()) {
				...
				return applyStaticFunctionCall(callExp, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.equals");
			}
		...
		case PredefinedType.SELECT_BY_KIND: {
			...
			sb.append("(java.util.Collection) ");
			sb.append("at.ac.tuwien.big.oclgen.OCL2JavaSupport.selectByKind");
			sb.append('(');
			sb.append(left);
			sb.append(", o -> {\n");
	...
	public String visitIteratorExp(IteratorExp<EClassifier, EParameter> callExp) {
		...
		try {
			...
			case PredefinedType.FOR_ALL:
				return applyFunctionPredicate(left, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.forAll", callExp, targetVarName, varNames);
			case PredefinedType.SELECT: 
				return applyFunctionPredicate(left, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.select", callExp, targetVarName, varNames);
			...
			case PredefinedType.COLLECT:
				if (Collection.class.isAssignableFrom(callExp.getBody().getType().getInstanceClass())) {
					return applyFunctionPredicate(left, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.collectCollection", callExp, targetVarName, varNames);
				} else {
					return applyFunctionPredicate(left, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.collect", callExp, targetVarName, varNames);
				}
			...
			case PredefinedType.CLOSURE:
				if (Collection.class.isAssignableFrom(callExp.getBody().getType().getInstanceClass())) {
					return applyFunctionPredicate(left, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.closureCollection", callExp, targetVarName, varNames);
				} else {
					return applyFunctionPredicate(left, "at.ac.tuwien.big.oclgen.OCL2JavaSupport.closure", callExp, targetVarName, varNames);
				}
			}
		} finally {
			for (int i = 0; i < varNames.length; i++) {
