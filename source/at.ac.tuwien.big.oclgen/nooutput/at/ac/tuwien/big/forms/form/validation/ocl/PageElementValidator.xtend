...
public class PageElementValidator extends org.eclipse.xtext.validation.AbstractDeclarativeValidator {
	...
	public static final String ELEMENTIDISUNIQUE_ISSUE_KEY = "ElementIDIsUniqueError";
	private static final at.ac.tuwien.big.oclgen.OCLBooleanExpressionEvaluator<PageElement> evaluatorElementIDIsUnique = at.ac.tuwien.big.forms.form.validation.ocl.ElementIDIsUniqueBooleanExpressionEvaluator.INSTANCE;
	
	@Check
	def void checkElementIDIsUnique(PageElement o) {
		if (!evaluatorElementIDIsUnique.isValid(o)) {
			val EStructuralFeature feature = evaluatorElementIDIsUnique.findErrorFeature(o);
			...
			error("The OCL-Expression named 'ElementIDIsUnique' evaluated to false!", feature, ELEMENTIDISUNIQUE_ISSUE_KEY, userData.toArray(newArrayOfSize(userData.size())));
		}
	}
}